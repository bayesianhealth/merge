"""
Databricks utility functions for the MIMIC-IV preprocessing pipeline.

Provides the same interface as the former snowflake_utils.py but backed by
SparkSession (which is pre-configured on Databricks clusters) and Unity Catalog.

Tables are expected to live in a Unity Catalog catalog named `mimiciv` with
schemas `hosp` and `icu` (matching the MIMIC-IV source layout).  Override the
catalog name via the MIMICIV_CATALOG environment variable if needed.
"""
import os
import pandas as pd
