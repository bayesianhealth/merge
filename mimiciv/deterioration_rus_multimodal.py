"""RUS (Relevance-Uniqueness-Synergy) computation for the Deterioration task.

Adapted from mimiciv_rus_multimodal.py to read from Delta tables instead of pkl files.
Computes Partial Information Decomposition (PID) between labs_vitals (48, 31)
and notes (48, 768) modalities for the deterioration prediction label.

Designed to run inside a Databricks notebook via runpy.run_path() or directly
as a cell. Requires an active SparkSession and the `pid` package in the path.

Usage in notebook:
    import runpy
    runpy.run_path(
        'deterioration_rus_multimodal.py',
        run_name='__main__',
        init_globals={'spark': spark}
    )

Or as a standalone script (will create its own SparkSession):
    python deterioration_rus_multimodal.py --num_subsample 5000 --seq_len 48 --num_lags 6
"""
...
# Add parent dir to path for the PID module
