...
# --- Large-model defaults (override at launch) ---
LARGE_DEFAULTS = dict(
    # Scaled architecture
    ...
    pos_weight=1.0,
    # FSDP
    use_mixed_precision=True, use_gradient_checkpointing=True,
    # Parallel strategy: "fsdp" (shard params) or "ddp" (replicate params)
...
def train_func():
    ...
    # --- Wrap for distributed training (FSDP or DDP) ---
    strategy = getattr(args, "parallel_strategy", "fsdp").lower()
    if is_distributed:
        if strategy == "ddp":
            from torch.nn.parallel import DistributedDataParallel as DDP

            # Gradient checkpointing under DDP requires disabling DDP's
            # static-graph buffer reuse; use find_unused_parameters for the
            # MoE router which may not route to all experts every step.
            model = DDP(
                model,
            ...
            if rank == 0:
                print("Model wrapped with DDP (replicated params, grad all-reduce)")
        else:
            ...
            model = FSDP(
                model,
                auto_wrap_policy=MultimodalTRUSMoEModel.fsdp_auto_wrap_policy(),
    ...
    # For distributed: use sharded dataset_map from context if available, else
    # fall back to streaming (for multimodal with notes, ShardedDataConnector
    # doesn't handle the ARRAY column well).
    dataset_map = context.get_dataset_map()
    use_dataset_map = dataset_map and "train" in dataset_map

    if use_dataset_map:
        # TS-only path via ShardedDataConnector
        ...
        # Multimodal path: each rank reads a disjoint Snowflake-side shard so the
        # notes path scales with GPU count (like the TS-only ShardedDataConnector).
        if rank == 0:
    ...
    for epoch in range(args.epochs):
        ...
        for ts, notes, note_mask, labels, weights in train_loader:
            ...
            # Replace skip with clamp to avoid NCCL deadlock in distributed training
            ...
            if args.clip_grad_norm > 0:
                if is_distributed and strategy != "ddp":
                    # FSDP exposes its own sharded-aware clip method
                    model.clip_grad_norm_(args.clip_grad_norm)
                else:
                    # DDP and single-GPU clip the local (replicated) params
        ...
        if rank == 0:
            ...
            if va_metrics["auroc"] > best_auroc:
                ...
                patience_counter = 0
                # Save checkpoint (unwrapped full state dict on rank 0)
...
def _estimate_params(args):
    ...
    d = args.d_model
    # Very rough: 2 modality encoders + shared transformer + MoE experts + classifier
...
def launch(session, ts_only=False, rus_path="results/deterioration",
           experiment_name="DETERIORATION_TRUS_MOE_LARGE", **overrides):
    """Launch distributed FSDP training from a notebook cell.
    ...
    # Scaling config: one worker (rank) per GPU so FSDP shards across all GPUs.
    # world_size = num_nodes * num_gpus; each worker holds exactly 1 GPU.
    ...
    print(f"Launching FSDP training: {num_nodes} node(s), {num_gpus} worker(s)/node, "
          f"1 GPU/worker (world_size={num_nodes * num_gpus})")
    print(f"Config: d_model={cfg['d_model']}, d_ff={cfg['d_ff']}, nhead={cfg['nhead']}, "
          f"experts={cfg['moe_num_experts']}, epochs={cfg['epochs']}")
